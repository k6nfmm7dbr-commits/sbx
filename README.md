...
深色控制台 / 浅色玻璃双主题，默认跟随系统，右上角可手动切换并记忆。面板保留真正有用的内容：

- 大号**实时速率数字**（上传/下载分项），无装饰波浪线
- TCP连接 / UDP会话 / 今日流量 / 累计流量
- 每日流量（7/30/90天）、每节点今日/累计、单节点每日明细、CSV导出
- 按用户要求，**彻底移除历史速率曲线**：前端模块、`/api/series`、`rate_samples` 持久化表、内存曲线缓存和所有曲线轮询均已删除

实时刷新优化：
- 实时速率走轻量 `/api/live`（2秒），历史概览走低频 `/api/summary`；所有请求防重入，慢网络不叠请求
- 数字短时缓动但不伪造数据；节点实时列只更新变化的文本，不整表重建
- 每日/节点图离屏构建后原子替换，不先清空闪白
- 页面后台暂停轮询、回前台立即刷新；手机顶栏不再sticky遮住内容
- 浏览器10秒实测：DOM节点数不增长、childList mutation=0、long task=0

实时速率底层仍使用v2真实耗时模型：每轮记录 `duration_ms`，严格按 `字节 ÷ 真实耗时` 计算；首次启动、规则换代、计数器归零的时长未知增量只进累计、不进实时样本，因此不会制造假瞬时速度。采集采用绝对周期调度，执行耗时不会累积漂移；samples只保留2分钟，降低SQLite写放大和磁盘占用。
