...
深色控制台 / 浅色玻璃两套高级主题，默认跟随系统，右上角可手动切换并记忆。顶部大号实时速率卡配迷你曲线，旁边是 TCP连接 / UDP会话 / 今日 / 累计四个关键指标。

- 速率曲线四档：5分 / 30分 / 2时 / 6时；**实线固定表示峰值，浅色面积固定表示时间加权平均**
- 每日流量（7/30/90天）、每节点今日/累计、TCP/UDP会话、单节点明细、CSV导出
- 高频 `/api/live` 与低频 `/api/summary` 分离；请求防重入，慢网络不叠请求
- 实时数字短时缓动；迷你曲线只更新 SVG path；历史图离屏构建后原子替换，不闪白
- 速率曲线走内存预聚合环缓存，真机4档API实测 **1.7–2.3ms**；10秒浏览器观测 DOM不增长、childList mutation=0、long task=0
- 手机顶栏不再 sticky 遮挡内容；后台暂停轮询，回前台立即刷新

### 真实速率模型

旧版只存“一个周期内字节”然后硬除配置2秒，卡顿、重启、首次基线都会算错。v2每轮记录**真实 duration_ms**，速率严格等于 `字节 ÷ 真实耗时`，并用绝对周期调度避免执行耗时累积漂移。

- 首次启动、规则换代、计数器归零：字节仍完整进入每日/累计，但时长未知，**绝不进入速率曲线**，消除几百MB/s假峰
- 刚安装多久就只显示多久：6小时档若只采集17分钟，左侧历史空白，并用“开始采集”标出起点
- Collector停摆/重建造成的缺口显示断线，不补0、不跨缺口连线
- 四档固定时间栅格：5分=5秒/点、30分=15秒、2时=1分钟、6时=3分钟
- 每桶自动同时计算：实线峰值（最高真实采样B/s）与浅色平均（按真实耗时加权），无需用户选择算法
- 全局 `rate_samples` 持久化 + 6小时内存预聚合，查询固定最多120桶，与节点数和运行时长无关

> 不做天/周/月速率曲线：长跨度看的是用量，不是瞬时速率；那是“每日流量”柱状图的职责。
