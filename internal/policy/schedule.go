...
//
// 配置范围严格对应完整四级滚轮：
//   - 日：1~30（每月最多配置到第 30 日）；
//   - 时：00~23（24 个值）；
//   - 分：00~59（60 个值）；
//   - 秒：00~59（60 个值）。
//
// 这里的「30 天」表示每个月可选择 1~30 日，而不是固定每 30*24 小时漂移一次。
// 调度采用自然日历月锚定：每月同一配置日、同一时分秒触发。2 月没有 29/30 日时
// 自动落到当月最后一天；进入下个月后恢复到用户配置日。
...
// ParseResetTime 解析规范 "HH:MM:SS" 为当天秒数（0..86399）。
// 定时重置精确到秒，三段都必须是两位；非法返回 -1。
