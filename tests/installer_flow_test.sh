...
run_deps_split() { # $1 = 缺失的工具（空格分隔），$2 = pkg_install 是否成功
  ...
  PKG="apt"
  # 桩 pkg_install 必须**真的落盘**建出文件：安装后的复检是 command -v，
  # 只返回 0 而不建文件的桩会让"装得上"场景被误判成失败。
...
# ---- 服务单元沙箱路径必须真实存在 ----
# 真机事故回归：ReadWritePaths 里写了 /run/sbx，而它在代码里从未被创建也从未
# 被使用（全仓只有那一处提及）。systemd < 248（Debian 11 / Ubuntu 20.04 /
# RHEL 8）对不存在的路径会让服务以 226/NAMESPACE 无限重启，systemd >= 248
# 则静默忽略——于是"新系统上没问题、老系统上面板永远起不来"。
sed -n '/^# >>> panel-unit/,/^# <<< panel-unit/p' "$TPL" > "$TMPD/unit.sh"
...
for rwp in $RW; do
  case "$rwp" in
    -*) ;;                        # "-" 前缀 = 允许缺失，systemd 会忽略
    /run/*|/tmp/*) BAD_RUN=1 ;;   # tmpfs：重启即清空，绝不能裸写
  esac
done
ck "ReadWritePaths 不含未加 - 前缀的 tmpfs 路径(/run、/tmp)" 0 "$BAD_RUN"
# 只看**代码/配置行**（剥掉 # 注释）：注释里保留这段历史说明是有价值的文档，
# 护栏要拦的是"配置里又把它写回去"，不是"不许提到它"。
sed 's/#.*$//' "$TPL" | grep -q '/run/sbx' && RB=1 || RB=0
ck "安装器配置不再引用 /run/sbx（注释中的历史说明除外）" 0 "$RB"
grep -q 'wait_service_active sbx-panel' "$TPL"; ck "安装后校验面板进入 active" 0 $?
