#!/usr/bin/env bash
# baseline_test.sh — v3.0.7 基线收口防回归：
...
ck "APP_VERSION == 3.0.7" "3.0.7" "$APP_VER"
ck "Go Version == 3.0.7" "3.0.7" "$GO_VER"
ck "README 当前版本 == 3.0.7" "3.0.7" "$README_VER"
ck "sbx.sh 版本 == 3.0.7" "3.0.7" "$SBX_VER"
...
for s in fresh_install_github e2e_remote fullinstall_remote; do
  grep -q '3\.0\.7' "$ROOT/scripts/$s.sh"; ck "终验脚本 $s 期望 3.0.7" 0 $?
done
...
ck "candidate version 3.0.3 != APP 3.0.7 → 拒绝" 1 "$([ "$RC" != 0 ] && echo 1 || echo 0)"
# success：3.0.7 → 允许
mk_candidate 3.0.7
( set +u; source "$TMPD/cv.sh"; install_sbx_core "$TMPD/cand" ) 2>/dev/null; RC=$?
ck "candidate version 3.0.7 == APP → 允许" 0 "$([ "$RC" == 0 ] && echo 0 || echo 1)"
# 无法解析的输出 → 拒绝（严格解析，非 substring）
printf '#!/bin/sh\necho "SBX Core version v3.0.7 linux amd64 build 123"\n' > "$TMPD/cand"; chmod +x "$TMPD/cand"
( set +u; source "$TMPD/cv.sh"; install_sbx_core "$TMPD/cand" ) 2>/dev/null; RC=$?
ck "非标准 version 输出 → 拒绝（严格解析）" 1 "$([ "$RC" != 0 ] && echo 1 || echo 0)"
# substring 陷阱：输出含 3.0.7 但实际版本不同 → 拒绝
printf '#!/bin/sh\necho "sbx-core v3.0.71"\n' > "$TMPD/cand"; chmod +x "$TMPD/cand"
( set +u; source "$TMPD/cv.sh"; install_sbx_core "$TMPD/cand" ) 2>/dev/null; RC=$?
ck "3.0.71 不得匹配 3.0.7（substring 陷阱）" 1 "$([ "$RC" != 0 ] && echo 1 || echo 0)"
# 真实 Go 二进制 version 输出可被解析为 3.0.7
# （iSH 等环境无法执行 Go 二进制——仓库 FUTURE_IMPROVEMENTS #6 已记录；CI/真机覆盖此检查）
REAL_VER=$( (cd "$ROOT" && go run ./cmd/sbx-core version 2>/dev/null) )
if [[ -n "$REAL_VER" ]]; then
  printf '%s\n' "$REAL_VER" | grep -q '^sbx-core v3\.0\.7$'; ck "go run ./cmd/sbx-core version == sbx-core v3.0.7" 0 $?
  echo "$REAL_VER" | sed -nE 's/^sbx-core v?([0-9]+\.[0-9]+\.[0-9]+)$/\1/p' | grep -qx '3.0.7'; ck "真实二进制输出可被 core_version_of 解析为 3.0.7" 0 $?
else
